...
class Admin extends CI_Controller{   // administrator controller
    ...
    public function index() // http://www.example.com/admin
    {
        ...
        if($this->session->userdata("session_token")!=null){// checking session token is null or not
            if($this->session->userdata("type")=="admin"){ // checking user is a admin or not
                ...
                if($resToken!=null && $this->User_Model->isValidToken($resToken)){ // checking response token is valid or not
                    redirect(base_url("/admin/loginSuccess")."?r=".$resToken); //redirecting to http://www.example.com/admin/loginSuccess with the response token
                }
                else{
                    $this->load->view('layout/admin_header',$data);
                    $this->load->view('admin_view/admin_login',$data); // loading admin login view
                }
            }else{
    ...
    function loginSuccess(){ //http://www.example.com/admin/loginSuccess
        ...
        if($this->User_Model->isValidToken($data["token"])) {
            redirect(base_url("admin/dashboard")); // redirecting to http://www.example.com/admin/dashboadr
        }else{
            redirect(base_url('admin/logout')); // then calling the logout url. http//www.example.com/admin/logout
        }
    }
   ...
   function dashboard(){  // http://www.example.com/admin/dashboadr
       ...
       if($this->session->userdata("session_token")!=null) {  // checking session token is null or not
           if($this->User_Model->isValidToken($resToken)){        // if session token is not null then checking the response token is valid or not
               $userId=$this->User_Model->getTokenToId($resToken);// obtaining the user id using the response token
               $view=true; // if the admin is superuser or 0 then view is true
               $adminType=(int)$this->Admin_Model->getAdminType($userId); //getting the admin type from database
               if($adminType==1){ //if the admin type is not 0 or 1
                   $view=false;  // admin is not a superuser. his role is a manager
               }

               $data['totalUser']=$this->Admin_Model->getTotalUser(); // getting the total number of users from the database
               $data['totalActiveUser']=$this->Admin_Model->getToatalActiveUser(); // getting the total number of active users from the database
               //$data['totalActiveUser']=rand(5,100);
               $data["totalMessage"]=$this->Admin_Model->getTotalMessage(); // getting the total number of messages from the database
               $data["totalGroups"]=$this->Admin_Model->getTotalGroup();// getting the total number of groups from the database
               $data["chart"]=$this->Admin_Model->getMonthelyMessage(date("Y")); // getting total messages monthly in this current year
               $data['view']=$view; // setting up the $view value
               $this->load->view("layout/admin_header"); // loading application/views/layout/admin_header.php
               $this->load->view("admin_view/common/navbar",$data);// loading application/views/admin_view/common/navbar.php and passing the $data variable to this view
               $this->load->view("admin_view/dashboard",$data);// loading application/views/admin_view/dashboard.php and passing the $data variable to this view
           }
    ...
    function logout(){ // http//www.example.com/admin/logout
        ...
        redirect(base_url('admin')); //redirecting to login page http://www.example.com/admin
    }
    ...
    function user($page=1){ // http//www.example.com/admin/user
        ...
        if($this->session->userdata("session_token")!=null) { // checking session token is null or not
            if($this->User_Model->isValidToken($resToken)){ // if session token is not null then checking the response token is valid or not
                $userId=$this->User_Model->getTokenToId($resToken); //obtaining user id from response token
                $view=true; // if the admin is superuser or 0 then view is true
                $adminType=(int)$this->Admin_Model->getAdminType($userId); //getting the admin type from database
                if($adminType==1){ //if the admin type is not 0 or 1
                    $view=false; // admin is not a superuser. his role is a manager
                ...
                if($filterData!=null || $filterData!=''){ //if the search variable is not empty
                    ...
                    if(!filter_var($filterData, FILTER_VALIDATE_EMAIL) === false) { // if the search variable contains an email address
                        $user = $this->Admin_Model->getUserByEmail($filterData); //then get the user data using that email address
                        $users=array();
                        if($user!=null){
                            $users[]=$user;
                        }
                        ...
                        if(count($fullName)>1){
                            $users=$this->Admin_Model->getUserByName($fullName[0],$fullName[1]);
                        }else{
                            $users=$this->Admin_Model->getUserByName($fullName[0],null);
                        }
                    ...
                    $this->load->view("admin_view/common/navbar",$data);
                    $this->load->view("admin_view/userList",$data);
                    //$this->load->view("admin_view/empty",$data);
                }else{
                    $this->load->library('pagination');
                    $config=$this->initPagination(base_url('admin/user'),$this->Admin_Model->getTotalUser());
                    $this->pagination->initialize($config);
                    $data["pagination_helper"]= $this->pagination;
                    $data["userList"]=$this->Admin_Model->getUserList($config['per_page'],(($page-1) * $config['per_page']));
            ...
            if($this->User_Model->isValidToken($resToken)){ // if session token is not null then checking the response token is valid or not
                ...
                if($filterData!=null || $filterData!=''){
                    ...
                    if(!filter_var($filterData, FILTER_VALIDATE_EMAIL) === false) {
                        $user = $this->Admin_Model->getUserByEmail($filterData);
                        $users=array();
                        if($user!=null){
                            $users[]=$user;
                        }
                    ...
                    $this->load->view("layout/admin_header");
                    $this->load->view("admin_view/common/navbar",$data);
                    $this->load->view("admin_view/messengerOptions",$data);
                    //$this->load->view("admin_view/empty",$data);
                }else{
                    $this->load->library('pagination');
                    $config=$this->initPagination(base_url('admin/messengerOptions'),$this->Admin_Model->getTotalUser());
                    $this->pagination->initialize($config);
                    $data["pagination_helper"]= $this->pagination;
                    $data["userList"]=$this->Admin_Model->getUserList($config['per_page'],(($page-1) * $config['per_page']));
                    $data['links']=$this->pagination->create_links();
            ...
            if($this->User_Model->isValidToken($resToken)){// if session token is not null then checking the response token is valid or not
                ...
                if($filterEmail!=null || $filterEmail!=''){
                    ...
                    if(!filter_var($filterEmail, FILTER_VALIDATE_EMAIL) === false) {
                        $users = $this->Admin_Model->getAdminByEmail($filterEmail);
                    }else{
                        $users=$this->Admin_Model->getAdminByName($filterEmail);
                    }
                ...
                $this->load->view("layout/admin_header");
                $this->load->view("admin_view/common/navbar",$data);
                $this->load->view("admin_view/admin_settings",$data);
                }else{
                    $this->load->library('pagination');
                    $config=$this->initPagination(base_url('admin/adminSettings'),(int)$this->Admin_Model->getTotalAdmin());
                    $this->pagination->initialize($config);
                    $data["pagination_helper"]= $this->pagination;
                    $data["userList"]=$this->Admin_Model->getAllAdmin($config['per_page'],(($page-1) * $config['per_page']));
                    $data['links']=$this->pagination->create_links();
                    $this->load->view("layout/admin_header");
                    $this->load->view("admin_view/common/navbar",$data);
                    $this->load->view("admin_view/admin_settings",$data);
    ...
    public function messenger($userID){
        ...
        if($this->session->userdata("session_token")!=null) {// checking session token is null or not
            if ($this->User_Model->isValidToken($resToken)) { // if session token is not null then checking the response token is valid or not
                if(ID_LOGIN){
                    $data["_r"]= json_encode($this->User_Model->getTokenRAWDataById($userID));
                    $data["T"]="RAW";
                }else{
                    $data["_r"]=$this->User_Model->getTokenById($userID);
                    $data["T"]="token";
                }

                $this->load->view("layout/header_messenger");
                $this->load->view("layout/navbar_empty_messenger",$data);
                $this->load->view("admin_view/messenger",$data);
    ...
    /*public  function support(){
        ...
        if($this->session->userdata("session_token")!=null) {// checking session token is null or not
            if ($this->User_Model->isValidToken($resToken)) {// if session token is not null then checking the response token is valid or not
                ...
                $this->load->view("layout/admin_header");
                $this->load->view("admin_view/common/navbar",$data);
                $this->load->view("admin_view/user_supportInfo",$data);
            }else{
                    redirect(base_url('admin/logout'));// redirecting to http://www.example.com/admin/logout
                }
            }else{
